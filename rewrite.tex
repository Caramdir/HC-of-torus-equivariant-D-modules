...
In particular, we will use the contraction principle of Drinfeld--Gaitsgory \cite{DrinfeldGaitsgory:2014:OnATheoremOfBraden}\remark{Better reference?}\ to understand the case of so-called contractive stacks.
...
\begin{Cor}\label{cor:properties_imply_monads}
  \remark{This needs to be fixed as we don't have endofunctors anymore.}
...
\begin{proof}
  This is just a restatement of Lemma~\ref{lem:pre:groupoid_monad_hol} applied to the construction in Section~\ref{sec:HH}.
\end{proof}
...
\begin{proof}
  By Sumihiro's theorem \cite{Sumihiro:1974:EquivariantCompletions}\remark{exact reference?} and Lemma~\ref{lem:open_cover} we can assume that $\stack X$ is affine with a linear action of $T$.
