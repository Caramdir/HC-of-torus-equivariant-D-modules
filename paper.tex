...
\subsection{A lemma on base change}\label{sec:base-change:base-change}
...
Using the notation of Section~\ref{sec:base-change:base-change} we set $\clsY{\stack X} = \bar{\stack X} ×_{\stack X × \stack X} \stack Y$ and $\lscY{\stack X} = {\stack X}^c ×_{\stack X × \stack X} \stack Y$.
...
\begin{Rem}
    ...
    For example, a direct computation shows that Theorem~\ref{thm:main} does not hold for the stack $\ps1/\as1$, and hence it is not \goodstack.
