...
\shorttitle{Hochschild cohomology of torus equivariant D-modules}

\author{Clemens Koppensteiner\affil{1}}
\abbrevauthor{C. Koppensteiner}
\headabbrevauthor{C. Koppensteiner}
\address{%
    \affilnum{1}Institute for Advanced Study, 1 Einstein Dr, Princeton, NJ 08540, USA
}
\correspdetails{clemens@ias.edu}
\received{}
%\date{April 9, 2018}
...
\begin{proposition}[{\cite[Section~4.2.1.3]{GaitsgoryRozenblyum:2017:StudyInDAG:2}}]
...
\begin{proposition}[{\cite[Section~2.5]{GaitsgoryRozenblyum:2014:CrystalsAndDModules}}]
...
\begin{equation}
    ...
    \quad\text{and}\quad
    \begin{tikzcd}
        T \arrow{r}{Tη} \arrow{d}[swap]{ηT} \arrow{dr}{\mathrm{id}} & T² \arrow{d}{μ} \\
        T² \arrow{r}[swap]{μ} & T
    \end{tikzcd}
\end{equation}
commute.
...
\begin{theorem}[{\cite[Corollary~C.5.4]{DrinfeldGaitsgory:2015:CompactGenerationOfDModOnBunG}}]
...
\begin{equation}\label{eq:diag:central1}
    \begin{tikzcd}
        \ls\stack X \arrow[hook]{r}{j_{\stack X}} \arrow{d}{p₂} & \cls\stack X \arrow{d}{\bar p₂} & \lsc\stack X \arrow[cm left hook->]{l}[swap]{i_{\stack X}}  \arrow{d} \\
        \stack X \arrow[hook]{r}{j} & \bar{\stack X} & \stack X^c \arrow[cm left hook->]{l}
    \end{tikzcd}
\end{equation}
For the second let $\stack Y₂ → \stack Y₁$ be a morphism of stacks over $\stack X$:
\begin{equation}\label{eq:diag:central2}
    \begin{tikzcd}
        \lsY[\stack Y₂]{\stack X} \arrow[hook]{r}{j_{\stack Y₂}} \arrow{d}{\ls f} & \clsY[\stack Y₂]{\stack X} \arrow{d}{{\cls f}} & \lscY[\stack Y₂]{\stack X} \arrow[cm left hook->]{l}[swap]{i_{\stack Y₁}}  \arrow{d} \\
        \lsY[\stack Y₁]{\stack X} \arrow[hook]{r}{j_{\stack Y₁}} & \clsY[\stack Y₁]{\stack X} & \lscY[\stack Y₁]{\stack X} \arrow[cm left hook->]{l}
    \end{tikzcd}
\end{equation}
...
\begin{proof}
    ...
    \begin{equation}
        ...
        \begin{tikzcd}
            \lsY[\stack Y₁]{\stack U} \arrow[hook]{r}{j_{\stack Y₁}^{\stack U}} \arrow[hook]{d}{α} &
            \clsY[\stack Y₁]{\stack U} \arrow[hook]{d}{β} &
            \clsY[\stack Y₂]{\stack U} \arrow{l}[swap]{{\cls f}^{\stack U}} \arrow[hook]{d}{γ} &
            \lscY[\stack Y₂]{\stack U} \arrow[cm left hook->]{l}[swap]{i^{\stack U}_{\stack Y₂}} \arrow[hook]{d}{δ} \\
            %
            \lsY[\stack Y₁]{\stack X} \arrow[hook]{r}{j_{\stack Y₁}^{\stack X}} &
            \clsY[\stack Y₁]{\stack X} &
            \clsY[\stack Y₂]{\stack X} \arrow{l}[swap]{{\cls f}^{\stack X}} &
            \lscY[\stack Y₂]{\stack X} \arrow[cm left hook->]{l}[swap]{i^{\stack X}_{\stack Y₂}}
        \end{tikzcd}
    \end{equation}
...
\begin{equation}
    ...
    \begin{tikzcd}
        Γ \arrow[hook]{r}{j'} \arrow{d} &
        \bar Γ \arrow{d} &
        \scls \stack X \arrow{l}[swap]{\bar p₂'} \arrow{d} &
        \slsc \stack X \arrow[cm left hook->]{l}[swap]{i_{\stack X}'} \arrow{d}\\
        %
        \stack X \arrow[hook]{r}{j} &
        \bar{\stack X} &
        \cls \stack X \arrow{l}[swap]{\bar p₂} &
        \lsc \stack X \arrow[cm left hook->]{l}[swap]{i_{\stack X}}
    \end{tikzcd}
\end{equation}
...
\begin{equation}
    ...
    \begin{tikzcd}
        \slsY[\stack Y₁]{\stack X} \arrow[hook]{r}{j_{\stack Y₁}'} \arrow{d} &
        \sclsY[\stack Y₁]{\stack X} \arrow{d} &
        \sclsY[\stack Y₂]{\stack X} \arrow{l}[swap]{\scls f} \arrow{d} &
        \slscY[\stack Y₂]{\stack X} \arrow[cm left hook->]{l}[swap]{i_{\stack Y₂}'} \arrow{d}\\
        %
        \lsY[\stack Y₁]{\stack X} \arrow[hook]{r}{j_{\stack Y₁}} &
        \clsY[\stack Y₁]{\stack X} &
        \clsY[\stack Y₂]{\stack X} \arrow{l}[swap]{\cls f} &
        \lscY[\stack Y₂]{\stack X} \arrow[cm left hook->]{l}[swap]{i_{\stack Y₂}}
    \end{tikzcd}
\end{equation}
...
\[
    \begin{tikzcd}
        \stack X \arrow[hook]{r}{{}^{H_i}j} &
        {}^{H_i}\bar{\stack X} &
        {}^{H_i}\clsY[\stack X]{\stack X} \arrow{l}[swap]{{}^{H_i}p_{2}} &
        {}^{H_i}\lscY[\stack X]{\stack X} \arrow[cm left hook->]{l}[swap]{{}^{H_i}i_{\stack X}}
    \end{tikzcd}
\]
and
\[
    \begin{tikzcd}
        \lsY[\stack Y₁]{\stack X} \arrow[hook]{r}{{}^{H_i}j_{\stack Y₁}} &
        {}^{H_i}\clsY[\stack Y₁]{\stack X} &
        {}^{H_i}\clsY[\stack Y₂]{\stack X} \arrow{l}[swap]{{}^{H_i}\cls f} &
        {}^{H_i}\lscY[\stack Y₂]{\stack X}. \arrow[cm left hook->]{l}[swap]{{}^{H_i}i_{\stack Y₂}}
    \end{tikzcd}.
\]
...
\bibitem{BensonIyengarKrause:2008:LocalCohomologyAndSupportForTriangulatedCategories}
...
\newblock \emph{Ann. Sci. \'Ec. Norm. Sup\'er. (4)},
  41\penalty0 (4):\penalty0 573--619, 2008.
\newblock ISSN 0012-9593.

\bibitem{Braverman:LecturesOnAlgebraicDmodules}
...
\bibitem{BravermanGaitsgory:2002:GeometricEisensteinSeries}
Alexander Braverman and Dennis Gaitsgory.
\newblock Geometric {E}isenstein series.
\newblock \emph{Invent. Math.}, 150\penalty0 (2):\penalty0 287--384,
  2002.
\newblock ISSN 0020-9910.
\newblock \doi{10.1007/s00222-002-0237-8}.

\bibitem{DrinfeldGaitsgory:2015:CompactGenerationOfDModOnBunG}
V.~Drinfeld and D.~Gaitsgory.
\newblock Compact generation of the category of {D}-modules on the stack of
  {$G$}-bundles on a curve.
\newblock \emph{Camb. J. Math.}, 3\penalty0 (1-2):\penalty0
  19--125, 2015.
\newblock ISSN 2168-0930.
\newblock \doi{10.4310/CJM.2015.v3.n1.a2}.

\bibitem{DrinfeldGaitsgory:2013:FinitenessQuestions}
Vladimir Drinfeld and Dennis Gaitsgory.
\newblock On some finiteness questions for algebraic stacks.
\newblock \emph{Geom. Funct. Anal.}, 23\penalty0 (1):\penalty0
  149--294, 2013.
\newblock ISSN 1016-443X.
\newblock \doi{10.1007/s00039-012-0204-5}.

\bibitem{DrinfeldGaitsgory:2014:OnATheoremOfBraden}
Vladimir Drinfeld and Dennis Gaitsgory.
\newblock On a theorem of {B}raden.
\newblock \emph{Transform. Groups}, 19\penalty0 (2):\penalty0 313--358,
  2014.
\newblock ISSN 1083-4362.
\newblock \doi{10.1007/s00031-014-9267-8}.

\bibitem{FrancisGaitsgory:2012:ChiralKoszulDuality}
John Francis and Dennis Gaitsgory.
\newblock Chiral {K}oszul duality.
\newblock \emph{Selecta Math. (N.S.)}, 18\penalty0 (1):\penalty0
  27--87, 2012.
\newblock ISSN 1022-1824.
\newblock \doi{10.1007/s00029-011-0065-z}.

\bibitem{GaitsgoryRozenblyum:2014:CrystalsAndDModules}
Dennis Gaitsgory and Nick Rozenblyum.
\newblock Crystals and {D}-modules.
\newblock \emph{Pure Appl. Math. Q.}, 10\penalty0
  (1):\penalty0 57--154, 2014.
\newblock ISSN 1558-8599.
\newblock \doi{10.4310/PAMQ.2014.v10.n1.a2}.

\bibitem{GaitsgoryRozenblyum:2017:StudyInDAG:1}
Dennis Gaitsgory and Nick Rozenblyum.
...
\bibitem{GaitsgoryRozenblyum:2017:StudyInDAG:2}
Dennis Gaitsgory and Nick Rozenblyum.
...
\bibitem{KashiwaraSchapira:1994:SheavesOnManifolds}
...
\bibitem{Keller:2006:OnDGCategories}
Bernhard Keller.
...
\bibitem{Lurie:2017-draft:HigherAlgebra}
Jacob Lurie.
\newblock \emph{Higher Algebra}.
\newblock URL \url{http://www.math.harvard.edu/~lurie/}.

\bibitem{Lurie:2009:HigherToposTheory}
Jacob Lurie.
...
\bibitem{Sumihiro:1974:EquivariantCompletions}
Hideyasu Sumihiro.
\newblock Equivariant completion.
\newblock \emph{J. Math. Kyoto Univ.}, 14:\penalty0
  1--28, 1974.
\newblock ISSN 0023-608X.
\newblock \doi{10.1215/kjm/1250523277}.
