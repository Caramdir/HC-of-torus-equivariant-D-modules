...
\begin{abstract}
    This article is dedicated to understanding the support theory (in the sense of \cite{BensonIyengarKrause:2008:LocalCohomologyAndSupportForTriangulatedCategories}) of equivariant D-modules.
...
\section{Base change for non-proper maps}
...
\subsection{A lemma on base change}\label{sec:d-mod:strategy:base-change}
...
\begin{proof}
    ...
    We split the adjunction in \eqref{eq:d-mod:base-change-adjunctions} in two by using the compositions
    \[
        f = \bar f ∘ j
        ,\
        p = \bar p ∘ \tilde\jmath
...
\begin{Rem}
    ...
    Thus it would suffice to assume that $X$ can be covered by open subschemes for which Lemma~\ref{lem:d-mod:finitely_many_stabilizers} holds.
\end{Rem}
...
\begin{proof}
    ...
    Thus the contraction principle Theorem~\ref{thm:d-mod:pre:contraction_principle} implies that
    \[
...
\begin{proof}[Proof of Theorem~\ref{thm:d-mod:torus:is-good}]
    By Lemma~\ref{lem:d-mod:stabilizers_cover} the closed subschemes $\schemeclsY[S]\stack X$ for $S ∈ \Stab \stack X$ cover $\schemeclsY\stack X$.
    If $S₁,\, S₂ ∈ \Stab\stack X$, then
    \[
        \schemeclsY[S₁]\stack X ∩ \schemeclsY[S₂]\stack X = 
        \schemeclsY[S₁∩S₂,\, S₁S₂].
    \]
    Thus the theorem follows from an iterated Mayer-Vietoris argument using Lemmas~\ref{lem:d-mod:key_for_stablizier}.
\end{proof}
