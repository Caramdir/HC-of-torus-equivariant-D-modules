...
For this let $V$ be a $G$-stable subvariety of $\bar G$ containing $G$, and let $\barGammasub{V}$ be the closure of $Γ$ in $V × X × X$.
We set
\[
...
Thus we have the following diagram which is central to remainder of this paper.
\[
    \begin{tikzcd}
    ...
    \end{tikzcd}
\]
Motivated by Lemma~\ref{lem:base-change-criterion} we make the following definition.
...
Theorem~\ref{thm:main} then immediately follows from the following theorem which lets us compute the Hochschild cohomology of $\catDMod{\stack X}$ for \goodstack\ quotient stacks.
...
\begin{proof}
    ...
    \]
    Setting $\stack Y = \stack X$ in Definition~\ref{def:good} it follows from Lemma~\ref{lem:base-change-criterion} that base change holds along the square
    \[
        \begin{tikzcd}
            \stack G₁ \arrow[r, "p_s"] \arrow[d, "p_t"] & \stack G₀ \arrow[d, "f"] \\
            \stack G₀ \arrow[r, "f"] & \stack X × \stack X
        \end{tikzcd}
    \]
    ...
    Thus a relative compactification of $\stack G_{m+1} → \stack G_m$ is given by $\stack G_m ×_{\stack X × \stack X} \bar{\stack X} → \stack G_m$.
    The map $j$ then identifies as the inclusion of $\stack G_m ×_{\stack X × \stack X} \stack X$.
...
\begin{proof}
    This follows from the fact that pullback along the smooth vertical morphisms in \eqref{eq:base-change:scheme-cover} is conservative \cite[Lemma~5.1.6]{DrinfeldGaitsgory:2013:FinitenessQuestions} and commutes with the other morphisms up to a shift.
\end{proof}
